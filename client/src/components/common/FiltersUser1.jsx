...
const FiltersUser1 = ({ closeModal }) => {
  const { filter, isLoading, users1 } = useSelector((state) => ({
    filter: state.filter,
    isLoading: state.app.loading,
    users1: state.users1.items
  }));
  ...
  const max = selectMax(users1);
  const min = selectMin(users1);
  ...
  return (
    <div className="filters">
      <div className="filters-field">
        ...
        <br />
        <br />
        {users1.length === 0 && isLoading ? (
          <h5 className="text-subtle">Loading Filter</h5>
        ) : (
          <select
            className="filters-brand"
            value={field.brand}
            disabled={isLoading || users1.length === 0}
      ...
      <div className="filters-field">
        ...
        <br />
        <br />
        {(users1.length === 0 && isLoading) || max === 0 ? (
          <h5 className="text-subtle">Loading Filter</h5>
        ) : users1.length === 1 ? (
...
FiltersUser1.propTypes = {
  closeModal: PropType.func.isRequired
};

export default withRouter(FiltersUser1);
